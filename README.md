...
Увага: ці обмеження в основному задаються **prompt-ом**, а не жорстким Python-лімітом.
...
На поточному етапі тут реалізовано **не agentic loop**, а більш контрольований режим перевірки.
...
## Поточні обмеження системи

1. Частина лімітів є тільки в prompt, а не в жорсткому Python-коді
2. Researcher інколи все ще може робити нерелевантні пошуки
3. `edit` реалізований як просте додавання feedback у content, а не повна інтелектуальна переробка звіту
4. Supervisor не є повністю ручною кодовою оркестрацією — частина контролю покладається на LLM

---

## Можливі подальші покращення

- Перенести цикл `plan → research → critique` у явний Python/StateGraph-контроль
- Зробити жорсткий кодовий ліміт revise-rounds
- Додати більш точний routing:
  - web-only
  - kb-only
  - mixed
- Покращити логіку `edit`, щоб feedback ішов назад у Supervisor як реальна ревізія звіту
- Посилити фільтрацію нерелевантних джерел у Researcher

---

## Підсумок

Ця програма є мультиагентною дослідницькою системою з:
- Planner
- Researcher
- Critic
- Supervisor
...
- веб-дослідження
- локальну базу знань
- критичну перевірку findings
- цикли доопрацювання
- контрольоване збереження звіту

У поточному вигляді система вже добре демонструє підхід **Plan → Research → Critique**, хоча частина обмежень ще базується на prompts, а не на повністю жорсткій кодовій оркестрації.
